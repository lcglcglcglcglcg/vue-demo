      ...
      this.lineString[i].forEachSegment(function (start, end) {
        ...
        this.styles.push(
          new Style({
            ...
            image: new Icon({
              src: 'http://a.amap.com/jsapi_demos/static/demo-center/icons/poi-marker-default.png',
              anchor: [0.75, 0.5],
              rotateWithView: true,
              rotation: -rotation,
            }),
 ...
 * */
  ...
  setLayers(mapType, visible) {
    ...
    if (mapType === 'openStreetMap') {
      ...
      descript = 'Map data &copy; <a href="http://www.openstreetmap.org" target="_blank">OpenStreetMap</a>'
      customLayers = this.setMapSource(path, descript, true)
      customLayers.set(mapType, mapType, true)
    } else if (mapType === 'gaoDeMap') {
      descript = 'Map data &copy; <a href="http://ditu.amap.com/" target="_blank">高德地图</a>'
      path = 'http://webrd0{1-4}.is.autonavi.com/appmaptile?size=1&scale=1&style=7&x={x}&y={y}&z={z}'
      customLayers = this.setMapSource(path, descript, visible)
      customLayers.set(mapType, mapType, true)
    } else if (mapType === 'gaoDeMapSate') {
      path = 'http://webst0{1-4}.is.autonavi.com/appmaptile?style=6&x={x}&y={y}&z={z}'
      descript = 'Map data &copy; <a href="http://ditu.amap.com/" target="_blank">高德地图</a>'
      customLayers = this.setMapSource(path, descript, visible)
      customLayers.set(mapType, mapType, true)
    ...
    let source = new XYZ({
      ...
      tileUrlFunction: function (tileCoord) {
        ...
        if (mapType == 'baiduMap') {
          ...
          return (
            'http://shangetu3.map.bdimg.com/it/u=x=' + x + ';y=' + y + ';z=' + z + ';v=009;type=sate&fm=46&udt=20170301'
          )
        }
      },
      attributions: ['Map data &copy; <a href="http://map.baidu.com" target="_blank">Baidu Map</a>'],
    })
